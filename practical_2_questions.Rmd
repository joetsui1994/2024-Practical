...
```{r aspatial, echo=TRUE,fig.keep = "none", results = "hide"}
...
```{r Gaussian shape, echo=TRUE, results = "hide", fig.keep = "none"}
...
```{r gravity, echo=TRUE, results = 'hide', fig.keep = "none", fig.cap='Spatial interaction matrix using parameters from Viboud et al. between all USA states.'}
require(ncf)
data(usflu)
...
```{r state, echo=TRUE,fig.keep = "none", results="hide", fig.cap='State level epidemics of Flu in the USA using a multipatch SIR model with parameters on spatial interactions based on Viboud et al. 2006. Initial case reported in NY state.'}
